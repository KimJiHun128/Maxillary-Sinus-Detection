...
def plot_images(images,
    ...
    for i in range(i + 1):
        ...
        #if paths:
        #    annotator.text((x + 5, y + 5), text=Path(paths[i]).name[:40], txt_color=(220, 220, 220))  # filenames 모든 이미지에 bbox, 글자가 안생김
        if len(cls) > 0:
            idx = batch_idx == i
    ...
    for f in files:
        try:
            ...
            for i, j in enumerate(index):
                ...
                ax[i].set_title(s[j], fontsize=12)  #ax[i].set_title("새로운 제목", fontsize=12)
